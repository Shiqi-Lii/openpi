#!/usr/bin/env bash
set -euo pipefail
# Run with: bash scripts/train_nz100_left.sh

# Modify these values for your left-arm training run.
DATA_REPO_ID="/mnt/16T/lisq5005_dir/openpi/data/data_nz100_left"
EXP_NAME="nz100_left_v1"
GPU_ID="0"
...
python scripts/compute_norm_stats.py \
    --config-name pi05_nz100_left \
    --repo-id "${DATA_REPO_ID}" \
    --assets-base-dir "${ASSETS_DIR}" \
    --num-workers "${NUM_WORKERS}" \
    --skip-videos

python scripts/train.py pi05_nz100_left \
